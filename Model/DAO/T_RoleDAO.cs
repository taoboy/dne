...
namespace com.gdce_task.Model
{
    ...
    /// </summary>
    public partial class T_RoleDAO:DAO
    {

        //处理本类的所有属性，把String类型里面的单引号处理一下
        public void AntiSQLInjection(T_Role entity) {
        ...
        public List<T_Role> DtToList(DataTable dt)
        {
            List<T_Role> entities = new List<T_Role>();
            foreach (DataRow dr in dt.Rows)
            {
                T_Role entity = new T_Role(dr, "");
                try
                {
        ...
        public int Add(T_Role entity)
        {
            ...
            sql.Append(" INSERT INTO [gdce_task].dbo.[T_Role] ( ");
            sql.Append(" [name],");
            sql.Append(" [descr],");
        ...
        public T_Role GetById(Int32 id)
        {
            T_Role entity = new T_Role() { id = id };
            return GetByModel(entity, false);
        }

        public int Del(Int32 id)
        {
            String sql = " DELETE FROM [gdce_task].dbo.[T_Role] WHERE [id] = @id";
            List<SqlParameter> paras = new List<SqlParameter>();
            paras.Add(new SqlParameter("@id", id));
            return DBHelper.ExecuteNonQuery(sql, paras);
        }

        public int Update(T_Role entity)
        {
            AntiSQLInjection(entity);
            ...
            sql.Append(" exec sp_executesql N'");
            sql.Append(" UPDATE [gdce_task].dbo.[T_Role] SET  ");
        ...
        /// </summary>
        public List<T_Role> Search(T_Role entity, int pageNum, int pageSize, String criteria, T_Role.Columns OrderBy, AscDesc AscDesc, out int totalRecords)
        {
            String sql = "";
        ...
        /// 以实体类作为查询条件获取符合条件的数据集，NULL值不作为查询条件
        /// </summary>
        public List<T_Role> GetAllByModel(T_Role entity) {
        ...
        public T_Role GetByModel(T_Role entity)
        {
        ...
        public T_Role GetByModel(T_Role entity, bool isLazy)
        {
            T_Role result = GetByModel(entity);
        ...
        /// </summary>
        private String PreparedTable(T_Role entity, bool isFuzzySearch, bool isSignleResult)
        {
            String sql = getFKColumnsSQL("[gdce_task].dbo.T_Role", T_Role.DBColumns, isFuzzySearch, isSignleResult);
            ...
            if (entity.id != null)
                sql += " and [gdce_task].dbo.T_Role.id = " + entity.id;

            if (entity.name != null)
                sql += String.Format(" and [gdce_task].dbo.T_Role.name = '{0}' ", AntiInjection(entity.name));

            if (entity.descr != null)
                sql += String.Format(" and [gdce_task].dbo.T_Role.descr = '{0}' ", AntiInjection(entity.descr));
